...
let privateKeyPem = process.env.ROOT_PRIVATE_KEY;
...
// Convert base64 DER to PEM format if needed
if (!privateKeyPem.includes('BEGIN RSA PRIVATE KEY')) {
  console.log('Converting base64 DER key to PEM format...');
  try {
    // Remove any whitespace/newlines
    const base64Clean = privateKeyPem.replace(/\s/g, '');
    // Convert to PEM format with proper line breaks
    const base64WithBreaks = base64Clean.match(/.{1,64}/g).join('\n');
    privateKeyPem = `-----BEGIN RSA PRIVATE KEY-----\n${base64WithBreaks}\n-----END RSA PRIVATE KEY-----`;
    console.log('✓ Key converted to PEM format');
  } catch (e) {
    console.error('ERROR: Failed to convert key format:', e.message);
    process.exit(1);
  }
}
