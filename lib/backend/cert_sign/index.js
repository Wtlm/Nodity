...
console.log('=== END SERVER KEY INFO ===');
...
app.post('/sign-cert', (req, res) => {
  ...
  // Self-verify to confirm signing works
  const mdVerify = forge.md.sha256.create();
  mdVerify.update(certContent, 'utf8');
  const selfVerified = publicKey.verify(mdVerify.digest().bytes(), signature);
  console.log('Server self-verification:', selfVerified);
